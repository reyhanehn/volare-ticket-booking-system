...
# this is a GET request this returns the list of available services in our database
# each vehicle can have a bunch of these services and how will be explained later
    path('admin/services/create/', CreateServiceView.as_view(), name='create_services'),
# this is a POST request this adds a service with the given name to the service table list
...
# this is a POST request for creating new company which only the admins are allowed to do
# (it will e checked from the permission class which we have a def to check the role of an account)
...
# this is a POST request here services are assigned to a specific vehicle
# {
#     "vehicle_id" : 1,
#     "service_names" : ["Meal", "Wifi"]
# }
]
